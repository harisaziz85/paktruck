...
@Component({
  selector: 'app-gradermachine',
  standalone: false,
  
  templateUrl: './gradermachine.component.html',
  styleUrl: './gradermachine.component.css'
})
export class GradermachineComponent {
  flatBedTrucks: any[] = [];
          ...
          this.flatBedTrucks = response.data
            .filter(ad => {
              ...
              return (
                ...
                ad.subCategory?.trim().toLowerCase() === 'grader truck' // Corrected subcategory
              );
            })
            .map(ad => ({

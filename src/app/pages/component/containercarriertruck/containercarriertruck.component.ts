...
@Component({
  selector: 'app-containercarriertruck',
  standalone: false,
  
  templateUrl: './containercarriertruck.component.html',
  styleUrl: './containercarriertruck.component.css'
})
export class ContainercarriertruckComponent {
 flatBedTrucks: any[] = [];
           ...
           this.flatBedTrucks = response.data
             .filter(ad => {
               ...
               return (
                 ...
                 ad.subCategory?.trim().toLowerCase() === 'container carrier truck' // Corrected subcategory
               );
             })
             .map(ad => ({
